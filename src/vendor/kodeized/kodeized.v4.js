...
if (location.hostname === "localhost" || location.hostname === "127.0.0.1"){
	  htmlTag.setAttribute("connection", "local");
	} else {
	  htmlTag.setAttribute("connection", "live");
}
/**./Detect Localhost connection**/
...
*/
bodyTag.classList.toggle('is_index', /\/$/.test(location.pathname));
	...
	*/
	jQuery(window).scroll(function(){
	  if (jQuery(this).scrollTop() > 100) {
		  jQuery('.scrollToTop').addClass("fade");
    } else {
	    jQuery('.scrollToTop').removeClass("fade");
	  }
	});
